...
const EditablePackageTable: React.FC<EditablePackageTableProps> = ({
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center py-24 bg-slate-50/50 dark:bg-[#1f1f1f]/50 rounded-3xl gap-6 border border-slate-100 dark:border-gray-800">
                <div className="w-10 h-10 border-2 border-slate-200 dark:border-gray-700 border-t-indigo-600 dark:border-t-indigo-500 rounded-full animate-spin"></div>
                <span className="text-slate-400 dark:text-gray-500 text-xs font-bold uppercase tracking-widest">
                    正在同步配置数据...
                </span>
            </div>
        );
    ...
    if (!products.length) {
        return (
            <div className="py-20 bg-white dark:bg-[#1f1f1f] rounded-3xl border-2 border-dashed border-slate-100 dark:border-gray-800 flex flex-col items-center">
                <Empty
                    description={
    ...
    const containerClasses = disabled
        ? 'w-full bg-white dark:bg-[#1f1f1f] rounded-2xl border border-slate-100 dark:border-gray-800 shadow-sm overflow-x-auto scrollbar-hide'
        : 'w-full bg-white/60 dark:bg-[#1f1f1f]/60 backdrop-blur-xl rounded-[2rem] border border-white dark:border-white/10 shadow-md overflow-x-auto scrollbar-hide';
