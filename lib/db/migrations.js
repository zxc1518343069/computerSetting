...
const DEFAULT_AFTER_SALES_CATEGORIES = [
    {
        code: 'diagnosis_system',
        name: '检测/系统',
        description: '电脑检测、系统安装、验机与桌面优化',
    },
...
const ensureSalesOrderSourceType = (database) => {
    const addedSourceType = ensureColumn(
        database,
        'sales_orders',
        'source_type',
    ...
    database.exec(`
        ...
        SET source_type = CASE
                WHEN source = 'frontend_quote' THEN 'diy'
                WHEN source = 'frontend_retail' THEN 'retail'
                WHEN source_type IN ('diy', 'retail', 'after_sales', 'manual') THEN source_type
                ELSE 'manual'
            END
        WHERE source_type IS NULL
           OR source_type = ''
           OR source_type NOT IN ('diy', 'retail', 'after_sales', 'manual')
           OR (${addedSourceType ? '1 = 1' : "source_type = 'manual'"}
               AND source IN ('frontend_quote', 'frontend_retail'));
...
const ensureSalesOrderAfterSalesTables = (database) => {
    database.exec(`
...
const ensureSalesOrderAfterSalesAdjustmentTables = (database) => {
    database.exec(`
...
const reclassifyLegacyManualSalesOrdersToDiy = (database) => {
    database.exec(`
        UPDATE sales_orders
        SET source_type = 'diy',
            updated_at = CURRENT_TIMESTAMP
...
const ensurePurchaseMerchantRefundInfrastructure = (database) => {
    database.exec(`
...
const migrations = [
    ...
    {
        id: '202606150001_sales_order_source_type',
        name: 'Add sales order source type',
        up: ensureSalesOrderSourceType,
    },
    {
        id: '202606150002_after_sales_order_items',
        name: 'Add after-sales order detail tables',
        up: ensureSalesOrderAfterSalesTables,
    },
    {
        id: '202606150003_after_sales_order_adjustments',
        name: 'Add after-sales order adjustment tables',
        up: ensureSalesOrderAfterSalesAdjustmentTables,
    },
    {
        id: '202606150004_reclassify_legacy_manual_sales_orders_to_diy',
        name: 'Reclassify legacy manual sales orders to DIY',
        up: reclassifyLegacyManualSalesOrdersToDiy,
    },
    {
        id: '202606160001_purchase_merchant_refunds',
        name: 'Add purchase merchant refunds',
        up: ensurePurchaseMerchantRefundInfrastructure,
    },
];
